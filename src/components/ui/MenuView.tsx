...
export function MenuView({
  onAddToCart, cartCount, total: _total, menuItems, categories, onOpenCart, location, onBack, config: _config,
}: MenuViewProps) {
  ...
  return (
    <div className="min-h-screen bg-dark-card pb-32 flex font-body overflow-x-hidden">
      {/* Desktop sidebar */}
      <aside className="w-56 bg-dark z-30 border-r border-white/10 overflow-y-auto p-5 space-y-1.5 pt-5 max-lg:hidden flex-shrink-0">
        {categories.map((cat) => (
      ...
      {cartCount > 0 && (
        <motion.button initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 0.9 }}
          whileTap={{ scale: 0.85 }}
          onClick={onOpenCart}
          className="fixed bottom-6 right-6 z-50 w-14 h-14 bg-gradient-to-r from-primary-vibrant to-secondary-vibrant rounded-2xl flex items-center justify-center shadow-2xl shadow-primary-vibrant/40 transition-all duration-200 active:scale-90"
        >
          <ShoppingCart className="w-6 h-6 text-white" />
          <span className="absolute -top-2 -right-2 bg-white text-dark text-[11px] font-bold w-6 h-6 rounded-xl flex items-center justify-center shadow-xl">
            {cartCount}
          </span>
        </motion.button>
      )}
