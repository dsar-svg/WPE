...
interface RestaurantContextType {
  ...
  createOrder: (order: Omit<Order, 'id' | 'created_at'>) => Promise<{ id: string; code: string }>;
...
export function RestaurantProvider({ children }: { children: ReactNode }) {
  ...
  const generateShortCode = useCallback(async (): Promise<string> => {
  ...
  const createOrder = useCallback(async (order: Omit<Order, 'id' | 'created_at'>): Promise<{ id: string; code: string }> => {
    try {
      const code = order.code || await generateShortCode();
