...
No test suite. Validate via browser + lint. MCP: TestSprite (in `opencode.jsonc`). Set `DISABLE_HMR=true` to disable HMR in dev.
...
On Vercel, `vercel-build.cjs` auto-detects project via `VERCEL_PROJECT_ID` env var: admin → `npm run build:admin`, pos → `npm run build:pos`, public → `npm run build`. Output is copied to `dist/` for Vercel to serve.

Supabase migrations applied via SQL Editor (not automated in CI).

**Gotcha**: `vercel link` overwrites `.env.local`. `deploy-all.ps1` backs it up before linking and restores after each deploy. Don't run `vercel link` manually without preserving `.env.local`.
